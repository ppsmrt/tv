...
import { getDatabase, ref, push, set, update, remove, onValue, get, child } from "https://www.gstatic.com/firebasejs/10.12.3/firebase-database.js";
...
onAuthStateChanged(auth, async (user) => {
  ...
  onValue(channelsRef, (snapshot) => {
    ...
    // ✅ Filter: only channels created by this user
    channelsData = Object.fromEntries(
      Object.entries(all).filter(([id, ch]) => ch.createdBy === user.email || ch.createdBy === user.uid)
    );
    renderChannels();
  });
});
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    let userId = user ? user.uid : "guest";
    let userEmail = user ? user.email : "unknown";
    ...
    const channelData = {
      ...
      tags,
      description,
      createdBy: userEmail, // ✅ store user identity
      ownerUid: userId
    };
...
function renderChannels() {
  ...
  if (!filtered.length) {
    channelList.innerHTML = "<p class='text-gray-500'>No channels found for your account.</p>";
    return;
  }
